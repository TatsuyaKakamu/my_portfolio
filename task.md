...
外部サービスとの自動連携・自動更新は当面行いません。すべて手動で更新します。
...
- GitHub / Instagram / Strava の連携は当面行わず、表示は仮データまたは「準備中」のままにする
- 将来的な自動更新は本タスクのスコープ外とする (現時点では計画しない)
...
- [ ] Strava の集計値を手動で更新するか、表示自体を非表示にする (自動取得はしない)
...
- Strava API による集計値の自動取得
- Instagram の公式APIまたは公式埋め込みによる自動表示
- TÜV SÜD 資格ページの自動チェック
- 上記の自動更新を行うための GitHub Actions の追加
- 上記の自動更新に必要な Secrets 管理

`src/data/portfolio.ts` の各データに含まれる `lastFetchedAt` / `lastCheckedAt` は、自動取得を前提とした命名だが、当面は「手動で最後に更新した日付」として扱う。
...
- [x] 外部API連携の自動更新を行うか → 当面行わない (手動更新)
