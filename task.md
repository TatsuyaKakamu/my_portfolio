...
- [x] 写真ギャラリーは実写真 5 枚 (`public/images/photos/*.jpg`) に差し替え済み
- [ ] プロフィール画像 / `MLX Audio Transcriptor` のスクリーンショットは生成画像のまま
...
- [x] `Photographer Demo Site` のスクリーンショット画像 (`website-photographer-demo.png`) を設定済み
- [ ] `MLX Audio Transcriptor` の更新日とスクリーンショットは仮データのまま (`Photographer Demo Site` は実 URL / 説明を反映済み)
- [x] GitHub / Instagram の外部リンクを有効化済み (TÜV SÜD は従来から有効)
- [ ] Strava の外部リンクは未設定 (準備中表示のまま)
- [ ] Strava の集計値は仮データ
- [x] 旧プロジェクト・旧フォトギャラリーで使っていたが現在参照されていない `public/images/generated/` 配下の画像 14 ファイルを削除済み
...
- [x] `.project-shot.is-placeholder` のスタイルを追加 (アスペクト比 3/2、薄いラベル + アイコン)。現状は両カードとも screenshot 画像を持つためプレースホルダは表示されないが、将来カードを追加した際のフォールバックとして残す
...
- [x] Web サイト制作枠の対象を確定し、サムネイル画像・URL・説明・技術スタックを設定する (`Photographer Demo Site`)
- [x] 写真ギャラリーを実写真に差し替える
...
- [x] GitHub の公開プロフィールURLを設定する
- [x] Apps & Websites の各カードに GitHub リンク・公開URL (必要ならデモリンク) を設定する
- [x] Instagram の公開アカウントURLを設定する
...
- [x] Web サイト制作枠で紹介する具体的な対象を確定する → `Photographer Demo Site` (https://github.com/TatsuyaKakamu/photographer_demo_site / https://tatsuyakakamu.github.io/photographer_demo_site/)
- [ ] 未完成のまま公開してよい範囲
- [ ] 仮画像・仮データのまま公開してよいか
...
- [x] `astro.config.mjs` をGitHub Pages用に変更する
- [x] `.github/workflows/deploy.yml` を追加する
- [x] `.gitignore` に `.astro/` と必要なら `dist/` を追加する
- [x] GitHub Pages公開方式に合わせてREADMEを更新する
- [x] `src/data/portfolio.ts` の `activityAreas[apps]` を `Apps & Websites` / `アプリ・Web制作` にリネームする
- [x] `src/data/portfolio.ts` の `profile.badges` を `App & Web Developer` にリネームする
- [x] `src/data/portfolio.ts` の `projects` を 2 件構成 (`MLX Audio Transcriptor` + Web サイト制作枠) に整理する
- [x] `src/pages/index.astro` の navigation ラベルと `SectionHeading` を `Apps & Web` / `Apps & Websites` に更新する
- [x] `.project-grid` を 2 カード前提のレイアウトに調整する
- [ ] `src/data/portfolio.ts` の仮データを実データに差し替える (プロフィール画像 / `MLX Audio Transcriptor` のスクリーンショット・更新日 / Strava 集計値が残課題)
- [ ] Strava の外部リンクを有効化する (GitHub / Instagram は対応済み)
- [x] Apps & Websites の各カードに GitHub リンク・公開URL (必要ならデモリンク) を追加する
- [ ] 画像を適切なサイズに圧縮・リサイズする
- [ ] 画像ファイル名と配置を整理する
- [ ] 画像の `alt` テキストを実画像に合わせて更新する
- [ ] CTA の `disabled` 表示を、リンク設定後に通常リンクへ変更する
